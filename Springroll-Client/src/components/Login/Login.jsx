...
const Login = () => {
  ...
  return (
    ...
    <form className="space-y-4">
      {!isLoginMode && <input type="text" placeholder="Name" className="w-full p-3 border-2 border-gray-300 outline-none placeholder-gray-400" />}
      ...
      {!isLoginMode && <input type="password" placeholder="Confirm Password" className="w-full p-3 border-2 border-gray-300 outline-none placeholder-gray-400" />}
      ...
      <p className="text-center text-gray-600">{isLoginMode ? "Don't have an account?" : "Already have an account?"} <a href="#" onClick={e => {e.preventDefault(); setIsLoginMode(!isLoginMode)}} className="text-gray-800 hover:underline">{isLoginMode ? "Sign Up" : "Login"}</a></p>
    </form>
  </div>
  );
}
